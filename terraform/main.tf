...
resource "null_resource" "backend" { #####11#####
    triggers = {
...
resource "null_resource" "backend_delete" { #####11#####
    triggers = {
        instance_id = module.backend.id # this will be triggered everytime instance is created
    }
    connection {
...
resource "aws_lb_listener_rule" "backend" {
  ...
  condition {
    host_header { #host path ---backend.rithinexpense.online context path ----- rithin.expense.online/backend
